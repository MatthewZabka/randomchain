...
Most famously, Erdos and Renyi introduced a model for random
...
percolation theory.  Our perspective is to understand topological constructions
from a topological perspective. To this end, we introduce a model for building
and understanding random chain complexes.

For a topological space, the singular chain complex provides a 
If the topological space has aadditional structure, like a CW or 
simplicial decomposition , then the singular chains can be replaced
...
\]
for all $m$. The transformations are subject to the constraint $A_{m-1} A_m = 0$,
and should be chosen according to $\varphi$. The latter means the following.
After fixing the
standard basis for $\Fqn{n_m}$, it suffices to construct random
matrices $n_{m-1} \times n_m$ matrices $A_m$. We do so by 
choosing matrix entries i.i.d. from the distribution $\varphi$ on
$\Fqn{}$.\mc{Add exposition about models for random chain complexes.}
...
be understood in terms of dimension
\begin{equation}
  \beta_m := \dim \left(\ker A_{m-1} / \im A_m \right)\, .

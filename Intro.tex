...
by-product of topological spaces, known as a chain complex. \mz{This is a good introductory paragraph.}
...
cohomology, and of course algebraic topology.\mz{Do we need sources here?} Specifically, chain
...
see~\cite{cartan2016homological} for further details. \mz{I've changed the citation to something more algebraic}
...
topology. \cite{hatcher2002algebraic} \mz{I've put the Hatcher citation here instead -- do we need more?} \mc{Add citations}.

We work over the field with $q$-elements $R = \Fqn{}$ and consider the chain complex whose $R$-modules are given by finite dimensional vector spaces,  $C_m = \Fqn{n_m}$, where $n_m$ is in $\mathbb{N}$. In this setting, the boundary maps can be regarded as $n_{m-1}\times n_m $ matrices, which we denote by $A_m$. Homology can then be understood in terms of dimension \mz{We had defined the Betti numbers in terms of $A_m$ without defining the $A_m$. What I've written is one possible fix.}
\begin{equation*}
...
on $\Fqn{}$. We then say that $(\Fqn{n_m}, A_m)$ is a {\em random chain complex}. \mz{Does $\varphi$ belong in this notation? $(\Fqn{}, A_m, \varphi)$?  What more exposition do we need?} \mc{Add exposition about models for random chain complexes.}
...
\mc{Explain why thm A is good.}\mz{We need to decide on the index and be consistent.  Should it be $m$ or $j$?} As Theorem A gives a formula for computing the distribution of the Betti numbers, it also leads to formulas for other probabilistic properties of $\beta_m$, such as its moments and variance.
...
\begin{remark}
  ...
  $N_m = 0$, and the homology is trivial in probability as $q \ra \infty$. \mz{Should we refer to a cor here?} 
\end{remark}
...
a different \mz{Is it really that different? The map $D$ is just one of the nilpotent matrices from $V$ into $V$ -- they're just choosing their matrix according to a different distribution.  Best to talk about this.} notion of random chain complex than the one described above.
...
In \cite{zabka2018random}, Z. studied the properties of a random Bockstein operation when regarded the connecting homomorphism given by a random boundary operation between two finite dimensional vector spaces over $\Fqn{}$. \mz{I can add more here if we decide that we indeed want Ginzberg's results in our paper}

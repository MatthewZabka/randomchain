...
\mz{We should talk about whether we need the word \textit{model}.}

\mz{The above definition is long. It's possible we'd like to define the sequence of maps/matrices $\{A_m\}$ first.}
...
Let $(\Fq^{n_m}, A_m)$ be a uniform random chain complex \mz{with $m$ in $\{0,1,2,\ldots\}$} and $A_0:\Fq^{n_0}\to 0$.  Then
\begin{align*}
